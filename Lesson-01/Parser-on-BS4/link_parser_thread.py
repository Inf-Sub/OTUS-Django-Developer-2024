"""
Это экспериментальный код, судя по результату, в нем явно присутствуют ошибки в работе с threads, из-за чего результат
в несколько раз меньше, чем при других вариантах парсинга.
"""
...
def recursive_link_fetch(
    ...
    if depth > 1:
        ...
        for link in links:
            if level < depth:
                thread = threading.Thread(target=recursive_link_fetch, args=(
                    link, depth - 1, output, file_path, level + 1
                ))
